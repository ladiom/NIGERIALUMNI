-- Fix missing user profile for ladiomole@hotmail.com
...
-- 5. Create user record if it doesn't exist
DO $$
DECLARE
    auth_user_id UUID;
    alumni_id TEXT;
BEGIN
    ...
    SELECT alumni_id INTO alumni_id FROM pending_registrations 
    ...
    IF NOT EXISTS (SELECT 1 FROM users WHERE id = auth_user_id) THEN
        ...
        VALUES (auth_user_id, 'ladiomole@hotmail.com', alumni_id, NOW());
        
        RAISE NOTICE 'Created user record for ladiomole@hotmail.com with alumni_id: %', alumni_id;
    ELSE
